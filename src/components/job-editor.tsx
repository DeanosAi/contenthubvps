...
export function JobEditor({ job, onSaved, onDeleted }: { job: Job; onSaved: () => void; onDeleted: () => void }) {
  ...
  const [liveUrl, setLiveUrl] = useState(job.liveUrl || '')
  ...
  async function save() {
    await fetch(`/api/jobs/${job.id}`, {
      ...
      body: JSON.stringify({ title, description, stage, priority, hashtags, platform, liveUrl, notes }),
    })
    setEditing(false)
    onSaved()
  }
  ...
  return (
    <div className="mt-3 space-y-2 rounded-lg border border-slate-800 bg-slate-900 p-3">
      ...
      <textarea className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm min-h-20" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" />
      <div className="grid grid-cols-2 gap-2">
        ...
        <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={platform} onChange={(e) => setPlatform(e.target.value)}>
          <option value="">No platform</option>
          {PLATFORMS.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
      </div>
      <input className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={liveUrl} onChange={(e) => setLiveUrl(e.target.value)} placeholder="Live post URL" />
      <textarea className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm min-h-24" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes / production details" />
